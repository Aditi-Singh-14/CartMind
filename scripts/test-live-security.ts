...
async function testSecurityWithAuthenticatedSession() {
  ...
  console.log(`Customer 1 (session): ${customer1.name} (ID: ${customer1.id})`);
  console.log(`Customer 2 (spoofed): ${customer2.name} (ID: ${customer2.id})`);

  // 2. Create auth user for customer1
  ...
  // 3. Link customer1 to this auth user
  ...
  // 5. Test /api/recommend with spoofed customer_id
  ...
  // 6. Test /api/decisions/[id]/respond with spoofed customer_id
  console.log('\n--- Testing /api/decisions/[id]/respond with spoofed customer_id ---');
  
  // Get a decision ID for customer1
  ...
  if (decisions && decisions.length > 0) {
    ...
    console.log('No agent_decisions found for customer1. Creating a test decision...');
