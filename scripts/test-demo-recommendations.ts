...
async function testDemoRecommendations() {
  ...
  // 1. Create demo account with order history
  ...
  // 3. Create historical orders for this customer
  const now = new Date();
  ...
  const runningGearProducts = products.filter(p => p.category === 'Running Gear');

  console.log(`Found ${skincareProducts.length} skincare products`);
  console.log(`Found ${runningGearProducts.length} running gear products`);
  ...
  const orderDefs = [
    ...
    // Order 3: Mixed items 15 days ago
    {
      ...
      items: [products[0], products[4]].map(p => ({ product: p, qty: 1 }))
    }
  ];
  ...
  for (const oDef of orderDefs) {
    ...
    console.log(`✓ Order created: ${insertedOrder.id} (${oDef.created_at})`);
  ...
  // 4. Check the customer's order history
  console.log('\n--- Checking customer order history ---');
  const { data: orders, error: ordersError } = await supabase
    .from('orders')
    .select('id, created_at, total_amount, order_items(product_id, products(name, category, price))')
  ...
  if (ordersError) {
    ...
    orders.forEach((order, idx) => {
      ...
      order.order_items.forEach((item: any) => {
        console.log(`    - ${item.products.name} (${item.products.category}) - ₹${item.products.price / 100}`);
      });
    });
  }
  ...
  // 6. Add 2 items to cart and call /api/recommend
  console.log('\n--- Testing recommendations with 2 cart items ---');
  
  const cartProduct1 = products[0];
  const cartProduct2 = products[1];
